...
test('skipped malformed ICU message json', async () => {
  await expect(
    ...
    stderr: expect.stringMatching(
      /^\[@soxhub\/formatjs-cli\] \[WARN\] Error validating message "my name is {name" with ID "a1dd2" in file .*\/packages\/cli\/integration-tests\/compile\/lang\/malformed-messages.json/
    ),
  } as Partial<any>)
}, 20000)
